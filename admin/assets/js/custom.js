...
app.row_add = function (that, data) {
    ...
    if (that.find('.' + data.gridBody + '> tr#' + ins_id).length <= 0 && app.checkdata(data, 'html')) {
        ...
        $('<tr class="entry" id="' + tr_id + '"></tr>').insertAfter('.' + data.gridBody + ' > tr#' + ins_id).html(hm);
    }
    ...
    if (tr.find('.dtin').length > 0) {
        tr.find('.dtin').html(' <textarea name="info[' + tr_id + '][description]" id="info' + tr_id + '_description" class="form-control"></textarea>');
    }
    app.tiny_mce('#' + tr.find('textarea').attr('id'));
