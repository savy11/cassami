<?php
 ...
 class controller extends main
 {
  ...
  public $sending_types = array(
   1 => 'CC Mail',
   2 => 'BCC Mail',
   3 => 'Prevent Mail'
  );
  public $per_levels = array(
   '1' => array(
    'View' => 'View',
   ),
   '2' => array(
  ...
  public function auth()
  {
   ...
   $query = "SELECT f.id, f.form_code, f.form_title, f.display_icon, f.per_level, p.per_view, p.per_add, p.per_edit, p.per_delete "
    . "FROM a_forms f "
    . "LEFT OUTER JOIN a_forms_per p ON f.id=p.form_id AND p.group_id='" . $this->user['group_id'] . "' "
    . "WHERE f.parent_id=0 AND f.form_code!='' "
    . "ORDER BY f.display_no";
   if ($forms = $this->db->selectall($query)) {
    foreach ($forms as $v) {
     $query = "SELECT f.id, f.form_code, f.form_title, f.seo, f.display_icon, p.per_view, p.per_add, p.per_edit, p.per_delete "
      . "FROM a_forms_per p "
      . "INNER JOIN a_forms f ON p.form_id=f.id AND p.group_id='" . $this->user['group_id'] . "' "
      . "WHERE f.parent_id='" . $v['id'] . "' AND p.per_view='1' "
      . "ORDER BY f.display_no";
  ...
  public function permalink($type = '', $data = array(), $app = false)
  {
   ...
   switch ($type) {
    case 'blog-detail':
     $url = app_url . 'blog' . ($data ? ds . $data['page_url'] . '-' . $data['blog_id'] : '');
     break;
    default:
     $url .= $type;
     break;
  ...
  {
   $where = "WHERE publish='Y'";
   if ($id != '') {
    $where .= " AND gallery_type LIKE '%$id%'";
   }
   $query = "SELECT * FROM m_blogs_cat {$where} ORDER BY id";
  ...
  function seo($data)
  {
   $arr = ['page_title' => 'Title', 'page_url' => 'Url', 'meta_keywords' => 'Keywords', 'meta_desc' => 'Description'];
   foreach ($arr as $k => $v) {
    echo '<label class="label label-' . ($this->varv($k, $data) && strlen($data[$k]) >= 3 ? 'success' : 'danger') . '" style="border-right: 1px solid;">' . $v . '</label>';
   }
  }
