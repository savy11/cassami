<?php
 ...
 class v_quotes extends controller {
  ...
  public function insert() {
   ...
   $data = $this->db->select("SELECT email FROM v_quotes WHERE id='" . $this->replace_sql($get_id) . "'");
   $id = $this->db->insert('v_quotes', array(
       'parent_id' => $get_id,
       'display_name' => 'Admin',
...
//   $this->send_email("contact_reply", $id);
  ...
  public function select() {
   $query = "SELECT * FROM v_quotes WHERE id='" . $this->replace_sql($this->get('id')) . "'";
   if (!$this->data = $this->db->select($query)) {
    $this->not_found();
   }
   $this->populate_post_data();

   $query = "SELECT * FROM v_quotes WHERE parent_id='" . $this->data['id'] . "' ORDER BY add_date";
