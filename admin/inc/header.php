  ...
  <nav class="navbar navbar-default top-header">
   <div class="container-fluid">
    <div class="navbar-header">
     ...
     <a class="navbar-brand text-logo" href="<?php echo $fn->permalink(); ?>"><?php echo str_replace('Home For All', '<span>Home For All</span>', app_name); ?></a>
    </div>
    <div class="collapse navbar-collapse">
     ...
     <ul class="nav navbar-nav navbar-right user-menu">
      <li class="dropdown">
       <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><img src="<?php echo $fn->file_exists($fn->user['image']) ? $fn->get_file($fn->user['image']) : $fn->permalink('assets/img/no-user.jpg'); ?>" class="img-circle user-img" alt="<?php echo $fn->user['display_name']; ?>" /><span class="angle-down ti-angle-down"></span></a>
       <ul class="dropdown-menu right-arrow">
        <li><a href="<?php echo $fn->permalink('profile'); ?>"><span class="icon ti-user"></span> Profile</a></li>
        <li><a href="<?php echo $fn->permalink('lock'); ?>" class="lock"><span class="icon ti-lock"></span> Lock Screen</a></li>
        <li><a href="<?php echo $fn->permalink('logout'); ?>"><span class="icon ti-power-off"></span> Logout</a></li>
       </ul>
      </li>
     </ul>
    </div>
  ...
  <div class="mobile-menu">
   <div class="main-profile">
    ...
    <div id="profile-nav" class="list-group collapse" aria-expanded="false">
     <a href="<?php echo $fn->permalink('profile'); ?>" class="list-group-item"><span class="icon ti-user"></span> Profile</a>
     <a href="<?php echo $fn->permalink('lock'); ?>" class="list-group-item lock"><span class="icon ti-lock"></span> Lock Screen</a>
     <a href="<?php echo $fn->permalink('logout'); ?>" class="list-group-item"><span class="icon ti-power-off"></span> Logout</a>
    </div>
   </div>
   <?php if ($fn->menus) { ?>
      ...
      foreach ($fn->menus as $k => $v) {
       ...
       if ($fn->varv('data', $v) != '') {
        ...
        <li<?php echo $sel ? ' class="active"' : ''; ?>>
         <a href="#col-<?php echo $i; ?>" data-toggle="collapse" aria-expanded="false"><span class="icon <?php echo $v['icon']; ?>"></span> <?php echo _($k); ?> <span class="pull-right angle-down ti-angle-down"></span></a>
      ...
      if ($fn->show_buttons) {
       if ($fn->per_add || $fn->per_edit) {
        if (in_array($fn->get('action'), array('add', 'edit', 'sort', 'view')) == false) {
         if ($fn->per_add) {
          ?>
          <a class="btn btn-success btn-sm" title="<?php echo _('Add New'); ?>" href="<?php echo $fn->get_action_url('add'); ?>"><i class="ti-plus"></i> <?php echo _('Add New'); ?></a>
         <?php } if ($fn->show_sort) { ?>
          <a href="<?php echo $fn->get_action_url('sort'); ?>" class="btn btn-info btn-sm"><?php echo _('Sort'); ?></a>
         <?php } if ($fn->show_search) {
          ?>
          <a href="javascript:;" class="btn btn-warning btn-sm" onclick="$('.search-panel').slideToggle(500);"><i class="ti-search"></i> <?php echo _('Search'); ?></a>
          <?php
         }
        } else {
         ?>
         <a class="btn btn-default btn-sm" title="<?php echo _('Back'); ?>" href="<?php echo $fn->return_ref(true); ?>"><i class="ti-arrow-left"></i> <?php echo _('Back'); ?></a>
         <?php
        }
