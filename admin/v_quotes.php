<?php
 ...
 if (($fn->per_edit && $fn->get('action') == 'view')) {
  if ($fn->get('action') == 'view' && $fn->get('id')) {
   $fn->select();
  }
  ?>
  <form id="data-frm" name="data-frm" method="post" class="form-validate" autocomplete="off" enctype="multipart/form-data">
    ...
    <div class="panel-body">
     <div class="row">
      <div class="col-md-6">
       ...
       <p><strong>Email : </strong><a href="mailto:<?php echo $fn->post('email'); ?>"><?php echo $fn->post('email'); ?></a></p>
       <p><strong>Phone : </strong><?php echo $fn->post('contact_no'); ?></p>
       <p><strong>Subject : </strong><?php echo $fn->post('subject'); ?></p>                  
      </div>
      <div class="col-md-6">
       <p><strong>Date : </strong> <?php echo $fn->dt_format($fn->post('add_date'), 'F d, Y h:i A'); ?></p>
   ...
   <?php /*<div class="panel panel-default panel-heading-fullwidth panel-borders">
    ...
    <div class="panel-body" style="padding-bottom: 15px;">
     <div class="comments">
      <div class="message">
       ...
       <small class="time"> <i class="ti-time"></i> on <?php echo $fn->dt_format($fn->post('add_date'), 'F d, Y h:i A'); ?> <i class="ti-check text-success"></i> </small>
      </div>
      <?php
      if ($fn->replies) {
       foreach ($fn->replies as $k => $v) {
        ...
        <div class="message<?php echo $v['type'] ? '' : ' front'; ?>">
         ...
         <small class="time"> <i class="ti-time"></i> on <?php echo $fn->dt_format($v['add_date'], 'F d, Y h:i A'); ?> <i class="ti-check text-success"></i> </small>
        </div>
        <?php
       }
      }
     ...
     <div class="add-comment">
      ...
      <textarea name="reply" id="reply" class="form-control" rows="3" required><?php echo $fn->make_html($fn->post('reply')); ?></textarea>
     </div>  
    </div>
       ...
       <tbody>
        ...
        foreach ($fn->data as $row) {
         ...
         <tr>
          ...
          <?php ob_start(); ?>
        <li><a href="<?php echo $fn->get_action_url('view', $row['id']); ?>"><span class="icon ti-eye"></span> <?php echo _('View'); ?></a></li>
        <?php
