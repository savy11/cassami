<?php
 ...
 class blog extends controller
 {
  ...
  public function blogs()
  {
   ...
   $query = "SELECT b.*, c.category_name, c.page_url as cat_url, YEAR(b.blog_date) as year, LPAD(MONTH(b.blog_date),2,0) as month, f.meta_value as blog_image "
    . "FROM m_blogs b "
    . "LEFT OUTER JOIN m_blogs_cat c ON b.category_id=c.id "
    . "LEFT OUTER JOIN files f ON b.blog_image=f.id "
    . "{$where} ORDER BY b.blog_date DESC";
  ...
  function blog()
  {
   $query = "SELECT b.*, c.category_name, c.page_url as category_url, YEAR(b.blog_date) as year, LPAD(MONTH(b.blog_date),2,0) as month, f.meta_value as blog_image " . "FROM m_blogs b "
    . "LEFT OUTER JOIN m_blogs_cat c ON b.category_id=c.id "
    . "LEFT OUTER JOIN files f ON b.blog_image=f.id "
    . "WHERE b.id='" . $this->replace_sql($this->get('id')) . "' AND b.publish='Y'";
   if (!$this->cms = $this->db->select($query)) {
    $this->not_found();
   }
   
   $this->cms['url'] = $this->permalink('blog-detail', $this->cms);
   if ($this->cms['url'] != $this->current_url()) {
    $this->redirect($this->cms['url']);
   }
   $this->meta = array('ogurl' => $this->cms['url'], 'ogtype' => 'article', 'ogtitle' => $this->cms['blog_title'], 'ogdesc' => $this->cms['blog_desc'], 'ogimage' => $this->get_file($this->cms['blog_image']));
