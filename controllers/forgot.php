<?php
 ...
 class forgot extends controller
 {
  ...
  public function check_forgot()
  {
   ...
   if ($this->post('forgot', 'email') == '') {
    throw new Exception('Please enter your email address.');
   }
   if (filter_var($this->post('forgot', 'email'), FILTER_VALIDATE_EMAIL) === false) {
    throw new Exception('Please enter valid email address.');
   }
   $data = $this->db->select("SELECT id, publish, email FROM users WHERE email='" . $this->replace_sql($this->post('forgot', 'email')) . "'");
   if (!$data) {
