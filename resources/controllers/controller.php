<?php
 ...
 abstract class controller
 {
  ...
  public $filter_type = array('I' => 'INPUT', 'S' => 'SELECT');
  ...
  public $gender = array('M' => 'Male', 'W' => 'Female', 'U' => 'Unknown');
  ...
  {
   $query = "SELECT * FROM cron_emails WHERE status='N' ORDER BY id LIMIT 10";
  ...
  public function permalink($type = '', $data = array())
  {
   ...
   switch ($type) {
    ...
    case 'blog-archive':
     ...
     break;
    case 'ad-cat':
     $url .= 'ads' . ($data['page_url'] ? ds . $this->url_string($data['page_url']) : '');
     break;
    case 'ad-subcat':
     $url .= 'ads' . ($data['page_url'] ? ds . $this->url_string($data['cat_url']) . ds . $this->url_string($data['page_url']) : '');
     break;
    case 'ad-detail':
     $url .= 'ad' . ($data['id'] ? ds . $this->url_string($data['ad_title']) . ds . $data['id'] : 's');
     break;
    case 'user-profile':
     $url .= 'user' . ($data['id'] ? ds . $this->url_string($data['page_url']) . ds . $data['id'] : 's');
     break;
    default:
     $info = pathinfo($url . $type);
     if (isset($info['extension']) && in_array($info['extension'], ['css', 'js'])) {
  ...
  public function show_price($amt, $cur = '', $round = 0)
  {
   if (!$cur) {
    $cur = $this->currency . ' ';
   }
  ...
  public function del_multiple_files($type, $table, $id)
  {
   $query = "SELECT id FROM files WHERE type_id='" . $id . "' AND type='" . $type . "' AND table_name='" . $table . "' AND deleted='N'";
   if ($data = $this->db->selectall($query)) {
    foreach ($data as $v) {
     $this->db_file_delete($v['id']);
    }
   }
  }
