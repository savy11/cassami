...
app.chosen = function (t) {
    var c = $('html:not(.no-chosen) select:not(.no-select)');
    if (!c.length > 0)
        return;
    if (t !== false) {
        t = true;
            ...
            if (that.parent().find('.chosen-container').length > 0) {
                ...
                if (app.checkdata(data, 'ajax')) {
                    that.ajaxChosen({
                        ...
                        dataType: 'json',
                        delay: 250,
                        data: {search: true, action: data.action},
        ...
        fv.each(function () {
            ...
            that.validate({
                ...
                errorPlacement: function (error, element) {
                    if (element.get(0).type == 'checkbox' || element.get(0).tagName == 'SELECT') {
                        error.appendTo(element.parent());
                    } else if (element.parent().hasClass('input-group') == true || element.parent().hasClass('custom-file') == true) {
...
app.tiny_mce = function (selecter) {
    ...
    l.require(['resources/vendor/colorpicker/css/colorpicker.css', 'resources/vendor/colorpicker/js/colorpicker.js'], function () {
        ...
        c.ColorPicker({
            ...
            onChange: function (hsb, hex, rgb) {
                c.val('#' + hex);
                if (app.checkdata(c.data(), 'output')) {
                    $('#' + c.data('output')).css('background-color', '#' + hex);
                }
            }
        })
    }, true);
